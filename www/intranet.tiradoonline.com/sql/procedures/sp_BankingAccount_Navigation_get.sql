IF EXISTS (SELECT NAME FROM sysobjects WHERE NAME = 'sp_BankingAccount_Navigation_get' AND TYPE = 'P')
	DROP PROCEDURE sp_BankingAccount_Navigation_get;
GO

CREATE PROCEDURE sp_BankingAccount_Navigation_get
	@UserID 		INT
AS
	...
	SET
		TotalBanking = (
					SELECT 
						ISNULL(SUM(a.Payment), 0) 
					FROM 
						Banking a 
					WHERE 
						BankingAccountID = #BankingAccount.BankingAccountID 					
						OR SubBankingAccountID = #BankingAccount.BankingAccountID
						AND BankingDate <= GETDATE()
				);
					-- OR
...
exec sp_BankingAccount_Navigation_get 1001
-- exec sp_BankingAccount_Navigation_get 1002
-- exec sp_BankingAccount_Navigation_get 1042
